...
def genPairedSineWaveParamsFile(
    SNRs=np.arange(-5, 15), SamplesPerBin=100, fc=75000, fs=2048000, Ns=500
):
    Params = np.zeros((len(SNRs),SamplesPerBin, 3))
    Noisy = np.zeros((len(SNRs),SamplesPerBin, Ns), dtype=complex)
    Ns = 500
    for SNR in SNRs:
        for i in np.arange(SamplesPerBin):
            ...
            waveOG = genSineWave(fc=fc, fs=fs, Ns=Ns, plotDebug=False)
            ...
            Params[SNR - (np.min(SNRs)), i,:] = [fc, np.real(waveScaled[0]), np.imag(waveScaled[0])]
